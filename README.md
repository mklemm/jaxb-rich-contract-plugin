...
3. **clone**: Will generate a simple deep "clone" method for the generated classes based on the heuristic that it only makes sense to traverse further down in the cloned object tree for members of types that are actually cloenable themselves.
	Also can generate a "partial clone" method, that takes a `PropertyTree` object which represents an include/exclude rule for nodes in the object tree to clone. Excluded nodes will not be cloned and left alone.
...
* **1.3.1**:    Also made group-interface work in an "episode" context, and fixed bug where empty interfaces were created if no implementation class for them could be found in the current module.
...
* Optionally, generates a "partial clone" method that takes a `PropertyTree` instance which represents a
  specification of the nodes in the object tree to clone. The PropertyTree is built up by an intuitive builder
  pattern:
...
Then, you would partially clone an object tree like this:
    ...
    final BusinessPartner businessPartnerClone = businessPartner.clone(excludeEmployees, PropertyTreeUse.EXCLUDE);
    
Which is the same as

	final BusinessPartner businessPartnerClone = businessPartner.cloneExcept(excludeEmployees);
	
This way, the clone of the original `businessPartner` will have no employees attached to the contained `company`.
It is also possible to clone only a specific subset of the original object tree, excluding everything else. The
inverse result of the above would be generated by:

	final BusinessPartner businessPartnerClone = businessPartner.clone(excludeEmployees, PropertyTreeUse.INCLUDE);
	
or

	final BusinessPartner businessPartnerClone = businessPartner.cloneOnly(excludeEmployees);
	
which will result in a businessPartnerClone where every property is set to null, except the company property,

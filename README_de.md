...
jaxb2-rich-contract-plugin ist ein Plugin fÃ¼r den XJC "XML to Java compiler" der JAXB API, die in allen Java SE-Versionen seit 1.6 enthalten ist.
Um das Plugin zu aktivieren, sind folgende Schritte erforderlich:

* JAR des Plugins zum Klassenpfad des XJC-Compilers hinzufÃ¼gen.
* Falls die Standard-JAXB-Version der Compiler-Umgebung kleiner als 2.2 ist, mÃ¼ssen die JAXB-Bibliotheken in Version 2.2 ebenfalls zum Klassenpfad hinzugefÃ¼gt werden.
* Kommandozeilenoption zur Aktivierung des gewÃ¼schten Plugins zur XJC-Kommandozeile hinzufÃ¼gen.
* Die meisten Plugins haben auÃŸerdem noch eigene Kommandozeilenoptionen. Diese mÃ¼ssen direkt nach der Aktivierungsoption ("-X...") angegeben werden, um Namenskonflikte zwischen Optionen verschiedener Plugins zu vermeiden.
* Die Plugins  "immutable" und "constrained-properties" schlieÃŸen sich gegenseitig aus und kÃ¶nnen nicht beide in einer einzigen Kommandozeile aktiviert werden.
  Ein Objekt kann nicht gleichzeitig unverÃ¤nderlich sein und Ã„nderungsnachrichten schicken.

####  Benutzung mit Apache Maven

Das "maven-jaxb2-plugin" der `<build>` -Konfiguration hinzufÃ¼gen. In dessen Konfigurationsabschnitt mÃ¼ssen dann die einzelnen Plugins aktiviert werden. Ebenso wird hier der Klassenpfad so gesetzt, dass bei der AusfÃ¼hrung von XJC das Plugin-JAR `jaxb2-rich-contract-plugin` im Klassenpfad ist.

Dieses "cheat sheet" gibt alle verfÃ¼gbaren Plugin-Optionen an und zeigt, wie die AbhÃ¤ngigkeit zum Plugin-JAR in den XJC-Klassenpfad konfiguriert wird:
...
```
Hinweis: Das Flag `<extension/>` muss auf "true" gesetzt sein, damit XJC Ã¼berhaupt plugins akzeptiert.

Hinweis: jaxb2-rich-contract-plugin implementiert JAXB und XJC APIs in der Version 2.2. Falls Sie mit einem Ã¤lteren JDK arbeiten, mÃ¼ssen sie diese Bibliotheken ebenfalls zum Klassenpfad hinzufÃ¼gen.
